...
pycfg(){ case "$1" in
autogen)echo "3.10|autogen-agentchat;autogen-ext[openai];autogenstudio|autogen_agentchat;autogen_ext;autogenstudio|0|";;
browser-use)echo "3.11|browser-use;playwright|browser_use;playwright|1|";;
crawl4ai)echo "3.10|crawl4ai;playwright|crawl4ai;playwright|1|crawl4ai-doctor";;
crewai)echo "3.10|crewai|crewai|0|";;
langflow)echo "3.10|langflow|langflow|0|";;
mem0)echo "3.10|mem0ai|mem0|0|";;esac; }

test_imports(){
 local i="$1" id="${IDS[$i]}" py="$VENVS/$id/bin/python";[[ -x "$py" ]]||return 1
 IFS='|' read -r min pkgs imports browser doctor<<<"$(pycfg "$id")";IFS=';' read -ra mods<<<"$imports"
 for m in "${mods[@]}";do "$py" -c "import $m" >/dev/null 2>&1||return 1;done
}
test_browser(){
 local i="$1" id="${IDS[$i]}";IFS='|' read -r min pkgs imports browser doctor<<<"$(pycfg "$id")"
 [[ "$browser" != 1 ]]&&return 0
 local py="$VENVS/$id/bin/python"
 "$py" -c 'from playwright.sync_api import sync_playwright
with sync_playwright() as p:
    b=p.chromium.launch(headless=True)
    b.close()
print("PLAYWRIGHT_OK")' >/dev/null 2>&1
}
test_python(){ test_imports "$1"&&test_browser "$1"; }
...
install_python(){
 ...
 IFS='|' read -r min pkgs imports browser doctor<<<"$(pycfg "$id")";local ver;ver="$($base -c 'import sys;print(".".join(map(str,sys.version_info[:3])))')"
 ...
 if [[ "$browser" == 1 ]];then
   log STEP "Instalando Chromium do Playwright para $name..."
   "$py" -m playwright install chromium|tee -a "$LOG"
 fi
 if [[ -n "$doctor"&&-x "$v/bin/$doctor" ]];then
   "$v/bin/$doctor" 2>&1|tee -a "$LOG"||log WARN "Diagnóstico retornou avisos; será usado o teste real do navegador."
 fi
 test_imports "$i"||{ log ERROR "$name: importação Python falhou.";return 1; }
 test_browser "$i"||{ log ERROR "$name: Chromium não iniciou.";return 1; }
}
...
install_node(){
 local i="$1" d
 test_node "$i"&&{ log OK "${NAMES[$i]} já está instalado.";return; }
 d="$(node_dir "$i")"||return 1
 has npm||return 1

 if [[ -f "$d/pnpm-lock.yaml" ]]&&has pnpm;then
   (cd "$d"&&pnpm install --frozen-lockfile)|tee -a "$LOG"
 elif [[ -f "$d/package-lock.json" ]];then
   set +e
   (cd "$d"&&npm ci --no-audit --no-fund) 2>&1|tee -a "$LOG"
   local npm_ci_status=${PIPESTATUS[0]}
   set -e

   if [[ $npm_ci_status -ne 0 ]];then
     log WARN "npm ci não pôde usar o lockfile atual. Tentando npm install com compatibilidade de peer dependencies..."
     (cd "$d"&&npm install --legacy-peer-deps --no-audit --no-fund) 2>&1|tee -a "$LOG"
   fi
 else
   (cd "$d"&&npm install --legacy-peer-deps --no-audit --no-fund) 2>&1|tee -a "$LOG"
 fi
